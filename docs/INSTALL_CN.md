...
1. 将 `DeepSeek-Harness-Studio-Setup-1.3.0-x64.exe` 复制到目标电脑。
2. 双击运行，在安装向导中选择安装路径；默认安装到当前 Windows 用户目录。
...
- 在“偏好设置 → 软件更新”中可检查 GitHub Release；下载通过 SHA-256 校验后，可确认原地覆盖升级。
...
- 若当前使用 v1.0，请直接运行一次 1.3.0 安装程序覆盖原安装目录。之后更新无需先卸载。
