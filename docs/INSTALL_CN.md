...
1. 将 `DeepSeek-Harness-Studio-Setup-1.07.8-x64.exe` 复制到目标电脑。
...
- 在“偏好设置 → 软件更新”中可检查 GitHub Release；自动线路严格等待国内社区镜像失败后再尝试 GitHub，同一时间只运行一个下载任务。也可选择仅国内镜像、仅 GitHub 或仅自定义镜像，下载通过 SHA-256 校验后可确认原地覆盖升级。
