...
DeepSeek Harness Studio 1.3.0 起支持应用内更新。程序会读取公开 GitHub 仓库的最新正式 Release，比较版本号，并由用户决定是否下载和安装。自 1.06.0 起，安装改为**静默覆盖升级**：确认后自动覆盖安装到原目录并重启应用，不再弹出安装向导。
...
从 1.3.0 开始，后续版本可在“偏好设置 → 软件更新”中完成检查、下载和覆盖安装；从 1.06.0 开始，点“立即更新”即静默覆盖安装并自动重启，无需重新走安装流程。
...
- 安装仍需用户点击“立即更新”确认；确认后静默覆盖安装到原目录并自动重启（`/S --updated`），不再显示安装向导，也不会删除工作区、会话、插件、Skills 或 ModLens 配置。首次安装（双击安装包）是向导式，可选择安装目录。
