...
contextBridge.exposeInMainWorld('harnessStudio', {
  ...
  plugins: {
    ...
    inspectSource: (source) => ipcRenderer.invoke('plugins:inspect-source', source),
    diagnose: () => ipcRenderer.invoke('plugins:diagnose'),
    install: (source) => ipcRenderer.invoke('plugins:install', source),
    update: (name) => ipcRenderer.invoke('plugins:update', name),
    repair: (name = '') => ipcRenderer.invoke('plugins:repair', name),
    remove: (name) => ipcRenderer.invoke('plugins:remove', name),
    toggle: (name, enabled) => ipcRenderer.invoke('plugins:toggle', { name, enabled }),
    openLocation: (name) => ipcRenderer.invoke('plugins:open-location', name),
